...
// Tipe data platform udah dihapus, imageUrl udah masuk
...
export default function OfferRow({
  ...
  // Auto-generate gambar placeholder kalau di database kosong
  ...
  // Logika Gambar: Pakai imageUrl dari database, kalau nggak ada pakai placeholder
  ...
  return (
    <>
      ...
      >
        <td className="p-4">
          <div className="flex items-center gap-4">
            {/* Gambar Game */}
            <div className="w-12 h-12 rounded-xl bg-surface-container border border-outline-variant flex items-center justify-center shadow-sm overflow-hidden flex-shrink-0">
              <img
                src={displayImage}
                alt={offer.gameName}
            ...
            <div>
              <div className="font-semibold text-primary text-base group-hover:text-blue-600 transition-colors">
                {offer.gameName}
              </div>
        ...
        <td className="p-4 text-center">
          {isTracked ? (
            <button
              disabled
              className="bg-surface-variant text-on-surface-variant px-5 py-2 rounded-lg text-sm font-semibold cursor-not-allowed shadow-inner"
            >
              Terlacak
            </button>
          ) : (
            <button
              ...
              className="bg-slate-900 text-white px-5 py-2 rounded-lg text-sm font-semibold hover:bg-slate-800 transition-colors shadow-md flex items-center gap-1 mx-auto"
            >
      ...
      {isModalOpen && (
        ...
        >
          <div
            className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col transform transition-transform"
          ...
          >
            ...
            <div className="p-6 border-t border-gray-100 bg-white">
              {isTracked ? (
                ...
                >
                  <input type="hidden" name="gameName" value={offer.gameName} />
                  {/* <input type="hidden" name="platform" value={offer.platform} /> --> UDAH DIHAPUS */}
                  <input
                    type="hidden"
                    name="offerwall"
                    value={offer.offerwall}
                  />
                  <input type="hidden" name="usdValue" value={offer.usdValue} />
                  <input
                    type="hidden"
                    name="requirement"
                  ...
                  />
                  {/* Lempar imageUrl biar pas di-track, gambar tetep nempel ke database Task */}
                  <input
                    type="hidden"
                    name="imageUrl"
